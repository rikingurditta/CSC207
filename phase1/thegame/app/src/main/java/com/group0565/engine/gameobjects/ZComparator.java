...
public class ZComparator implements Comparator<UUID> {
    ...
    /**
     * Compare the two GameObjectUUIDs based on the z value of the GameObjects represented by
     * the UUIDs. To maintain consistency with equals, if the z value of the two objects are equal,
     * the UUIDs are compared using the compareTo method of the UUIDs.
     * <p>
     * Note that since the uuid is only a weak reference to the GameObjects, the GameObject they reference
     * may be null. UUIDs with null objects will always be smaller than those with objects.
     *
     * @param o1 First uuid
