...
public class TsuMenu extends GameObject implements Observer, Observable {
  ...
  public void init() {
    ...
    this.titleButton =
        new Button(
            new Vector((cx - TITLE_SIZE) / 2, (cy - TITLE_SIZE) / 2),
            new Vector(TITLE_SIZE, TITLE_SIZE),
            title,
            title);
    ...
    this.settingsMenu =
        new SettingsMenu(
            new Vector((cx - SETTINGS_SIZE) / 2, (cy - SETTINGS_SIZE) / 2),
    ...
    Bitmap settingsButtonBitmap =
        getEngine().getGameAssetManager().getTileSheet("Tsu", "Buttons").getTile(1, 0);
    this.settingsButton =
        new Button(
            new Vector(cx - BUTTON_SIZE - MARGIN, cy - BUTTON_SIZE - MARGIN),
            new Vector(BUTTON_SIZE, BUTTON_SIZE),
            settingsButtonBitmap,
            settingsButtonBitmap);
    settingsButton.registerObserver(this);
    adopt(settingsButton);

    Bitmap languageButtonBitmap =
        getEngine().getGameAssetManager().getTileSheet("Tsu", "Buttons").getTile(4, 0);
    this.languageButton =
        new Button(
            new Vector(cx - BUTTON_SIZE - MARGIN, MARGIN),
            new Vector(BUTTON_SIZE, BUTTON_SIZE),
            languageButtonBitmap,
            languageButtonBitmap);
    languageButton.registerObserver(this);
    adopt(languageButton);

    Bitmap statsBitmap =
        getEngine().getGameAssetManager().getTileSheet("Tsu", "Buttons").getTile(7, 0);
    this.statsButton =
        new Button(
            new Vector(MARGIN, cy - BUTTON_SIZE - MARGIN),
            new Vector(BUTTON_SIZE, BUTTON_SIZE),
            statsBitmap,
            statsBitmap);
