...
class HistoryList extends GameMenu {
  ...
  private static final String[] HistoryDisplayersName = {
    "HitoryDisplayer0", "HitoryDisplayer1", "HitoryDisplayer2", "HitoryDisplayer3"
  };
  ...
  public void init() {
    ...
    this.build()
        .add(
            ScrollUpName,
            new Button(SCROLL_BUTTON_SIZE, ScrollBitmap.SCROLL_UP.getBitmap(), -1)
                .build()
        ...
        .add(
            ScrollUpDsbName, new BitmapDrawer(null, ScrollBitmap.SCROLL_UP_DSB::getBitmap).setZ(-1))
        ...
        .add(
            ScrollDownName,
            new Button(SCROLL_BUTTON_SIZE, ScrollBitmap.SCROLL_DOWN.getBitmap(), 1)
                .build()
                .registerObserver(this::observeScroll)
                .setSelfEnable(
                    () -> (getHistory() != null && scroll < getHistory().size() - DISPLAYER_COUNT))
                .close())
        .addAlignment(Left, THIS, Left)
        .addAlignment(Right, THIS, Right)
        ...
        .add(
            ScrollDownDsbName,
            new BitmapDrawer(null, ScrollBitmap.SCROLL_DOWN_DSB::getBitmap).setZ(-1))
  ...
  private void observeScroll(Observable observable, ObservationEvent<Integer> event) {
    if (event.isEvent(Button.EVENT_DOWN)) this.setScroll(scroll + event.getPayload());
  }

  private void observeStatsMenu(Observable observable, ObservationEvent event) {
    if (event.getMsg().equals(StatsMenu.HISTORY_UPDATE_EVENT)) updateScroll();
  }
  ...
  public void setSelectedObject(SessionHitObjects selectedObject) {
    ...
    if (getHistory() != null && getHistory().contains(this.selectedObject)) {
      ...
      if (!(scroll <= index && index < scroll + DISPLAYER_COUNT)) setScroll(index);
    }
  ...
  private class HistoryBox extends GameMenu {
    ...
    public void init() {
      ...
      if (count > 1) {
        for (int i = 0; i < count; i++) {
          builder
              .add(
                  HistoryDisplayersName[i],
                  displayers[i] =
                      new HistoryDisplayer(new Vector(0, height), null, () -> selectedObject))
              .addAlignment(Left, THIS, Left)
              .addAlignment(Right, THIS, Right);
          displayers[i].registerObserver(this::buttonPressed);
          if (i == 0) builder.addAlignment(Top, THIS, Top);
          else builder.addAlignment(Top, HistoryDisplayersName[i - 1], Bottom);
        }
      }
      builder.close();
    }
    ...
    public void updateScroll() {
      ...
      for (int i = scroll; i < scroll + count; i++) {
        ...
        else displayers[i - scroll].setObjects(null);
      }
    }
  }
}
