...
public class Judgementer extends GameMenu {
  ...
  protected void onEventCapture(InputEvent event) {
    ...
    for (int i = passedPointer.getValue(); i < hitObjects.getValue().size(); i++) {
      ...
      float x =
          (float)
              (getAbsolutePosition().getX()
                  + (object.getPosition() * getSize().getX())
                  - width / 2f);
      // If it is outside the width of the object, continue
      if (event.getPos().getX() < x || x + width < event.getPos().getX()) continue;
      if (object.getHitTime() > 0) continue;
      // Otherwise see if the timing is correct
      if (computeHitScore(
              object.getMsStart(),
              currentTime.getValue(),
              calculateDistribution(averageDifficulty(beatmap.getValue().getDifficulty())),
              false)
          != null) {
      ...
      if (object.getMsStart() > currentTime.getValue()) break;
    }
  ...
  protected void onEventDisable(InputEvent event) {
    ...
    ArchiveInputEvent archiveInputEvent =
        activeArchiveMap.remove(event); // Remove the event from the active map
  ...
  public void draw(Canvas canvas, Vector pos, Vector size) {
    ...
    if (!(hitObjects == null || beatmap == null)) {
      ...
      for (int y = 0; y >= -GLOW_HEIGHT; y--) {
        for (InputEvent event : hitMap.keySet()) {
          ...
          if (object != null) drawGlowLine(canvas, pos, size, y, object);
        }
    ...
    Vector incrementSize =
        size.elementMultiply(new Vector(0.5f * beatmap.getValue().getNoteWidth(), 0));
    canvas.drawLine(
        pos.subtract(incrementSize), pos.add(size.newSetY(0)).add(incrementSize), judgementPaint);
  }
