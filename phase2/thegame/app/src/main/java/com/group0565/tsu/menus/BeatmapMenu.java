...
public class BeatmapMenu extends GameMenu {
    ...
    public void init() {
        ...
        ThemedPaintCan namePaint = new ThemedPaintCan(SET, BeatmapPaintName).init(getGlobalPreferences(), getEngine().getGameAssetManager());
        ThemedPaintCan artistPaint = new ThemedPaintCan(SET, ArtistPaintName).init(getGlobalPreferences(), getEngine().getGameAssetManager());
        ThemedPaintCan creatorPaint = new ThemedPaintCan(SET, CreatorPaintName).init(getGlobalPreferences(), getEngine().getGameAssetManager());
        ...
        LanguageText artistLabel = new LanguageText(getGlobalPreferences(), getEngine().getGameAssetManager(), SET, ArtistLabelName);
        LanguageText creatorLabel = new LanguageText(getGlobalPreferences(), getEngine().getGameAssetManager(), SET, CreatorLabelName);
        LanguageText difficultyLabel = new LanguageText(getGlobalPreferences(), getEngine().getGameAssetManager(), SET, DifficultyLabelName);

        // @formatter:off
        this.build()
            .add(BackgroundName, new BitmapDrawer(new Vector(), () -> (getSelectedBeatmap() == null ? null : getSelectedBeatmap().getBackground()), true).setZ(-1))
            .addAlignment(Left, THIS, Left)
            .addAlignment(Right, THIS, Right)
            .addAlignment(Top, THIS, Top)
            .addAlignment(Bottom, THIS, Bottom)

            .add(BackName, new Button(ButtonSize, ButtonBitmap.BackButton.getBitmap()).build()
                .registerObserver(this::observeBackButton)
                .close())
            .addAlignment(Left, THIS, Left, MARGIN.getX())
            .addAlignment(Top, THIS, Top, MARGIN.getY())

            .add(BeatmapName, new TextRenderer(() -> getSelectedBeatmap() == null ? "Beatmap not found" : getSelectedBeatmap().getName(), namePaint))
            .addAlignment(Left, THIS, Left, MARGIN.getX())
            .addAlignment(Top, BackName, Bottom, MARGIN.getY())

            .add(ArtistName, new TextRenderer(() -> artistLabel.getValue() + ": "
                    + (getSelectedBeatmap() == null ? "" : getSelectedBeatmap().getArtist()), artistPaint))
            .addAlignment(Left, THIS, Left, MARGIN.getX())
            .addAlignment(Top, BeatmapName, Bottom, INTERNAL_MARGIN.getY())

            .add(CreatorName, new TextRenderer(() -> creatorLabel.getValue() + ": "
                    + (getSelectedBeatmap() == null ? "" : getSelectedBeatmap().getCreator()), creatorPaint))
            .addAlignment(Left, THIS, Left, MARGIN.getX())
            .addAlignment(Top, ArtistName, Bottom, INTERNAL_MARGIN.getY())

            .add(DifficultyName, new TextRenderer(() -> difficultyLabel.getValue() + ": "
                    + (getSelectedBeatmap() == null ? "" : String.valueOf(getSelectedBeatmap().getDifficulty())), difficultyPaint))
            .addAlignment(Left, THIS, Left, MARGIN.getX())
            .addAlignment(Top, CreatorName, Bottom, INTERNAL_MARGIN.getY())

            .add(PrevName, new Button(ButtonSize, ButtonBitmap.PrevButton.getBitmap(), -1).build()
                .registerObserver(this::observeScrollButton)
                .close())
            .addAlignment(Left, THIS, Left, MARGIN.getX())
            .addAlignment(Bottom, THIS, Bottom, -MARGIN.getY())

            .add(NextName, new Button(ButtonSize, ButtonBitmap.NextButton.getBitmap(), 1).build()
                .registerObserver(this::observeScrollButton)
                .close())
            .addAlignment(Right, THIS, Right, -MARGIN.getX())
            .addAlignment(Bottom, THIS, Bottom, -MARGIN.getY())

            .add(PlayName, new Button(ButtonSize, ButtonBitmap.PlayButton.getBitmap()).build()
                .registerObserver(this::observePlayButton)
                .close())
            .addAlignment(Right, THIS, HCenter, -ButtonSize.getX())
            .addAlignment(Bottom, THIS, Bottom, -MARGIN.getY())

            .add(StatsName, new Button(ButtonSize, ButtonBitmap.StatsButton.getBitmap()).build()
