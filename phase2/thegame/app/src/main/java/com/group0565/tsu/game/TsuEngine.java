...
import static com.group0565.tsu.enums.Scores.S0;
...
public class TsuEngine extends GameMenu {
  ...
  public void init() {
    ...
    this.build()
        .add(
            ...
            (tsuRenderer =
                new TsuRenderer(
                    new Vector(PlayAreaWidth, 0),
                    this::getPassedPointer,
                    this::getCurrentTime,
                    this::getHitWindow,
                    this::getHitObjects,
                    this::getBeatmap)))
        ...
        .add(
            ...
            (judgementer =
                    new Judgementer(
                        new Vector(PlayAreaWidth, HIT_AREA),
                        this::getPassedPointer,
                        this::getBeatmap,
                        this::getHitObjects,
                        this::getCurrentTime))
        ...
        .add(
            PauseMenuName,
            (pauseMenu = new PauseMenu(PauseMenuSize))
                .build()
                .setZ(4)
        ...
        .add(
            PauseButtonName,
            new Button(PauseButtonSize, ButtonBitmap.PauseButton.getBitmap())
                .setZ(5)
                .build()
  ...
  public void update(long ms) {
    ...
    if (running) {
      ...
      // When our lead in almost ends, start the audio, accounting for the delay with starting the
      // audio
      ...
      while (passedPointer < hitObjects.size()
          && hitObjects.get(passedPointer).getMsEnd() < tsuRenderer.getScreenTime()) {
    ...
    if (event.isEvent(Judgementer.NOTE_HIT)) {
      lastHit =
          ScoreCalculator.computeScore(
              event.getPayload(),
              ScoreCalculator.calculateDistribution(averageDifficulty(beatmap.getDifficulty())),
              true);
      if (lastHit == S0) combo = 0;
      else {
        combo += 1;
  ...
  private void observePauseMenu(Observable observable, ObservationEvent<String> event) {
    if (event.isEvent(Button.EVENT_DOWN)) {
      if (event.getPayload().equals(PauseMenu.RESUME)) resumeGame();
      else if (event.getPayload().equals(PauseMenu.HOME)) exitGame();
    }
  }
    ...
    if (beatmap != null && hitObjects != null && audio != null) {
      ...
      if (generator == null && getGlobalPreferences() instanceof TsuPreferences) {
        ...
        if (preferences.getAuto()) {
          generator =
              new AutoGenerator(this, judgementer.getAbsolutePosition(), judgementer.getSize());
          intercepter.setGenerator(generator);
        }
      }

      if (generator != null) generator.init();
  ...
  private void endGame() {
    ...
    SessionHitObjects sessionHitObjects =
        ScoreCalculator.constructSessionHitObjects(beatmap, judgementer.getArchive());
    if (getGlobalPreferences() instanceof TsuPreferences)
    ...
    this.notifyObservers(
        new ObservationEvent<>(
            StatsMenu.TO_REPLAY.equals(source) ? TO_STATS : GAME_END, sessionHitObjects));
    this.source = null;
  }
  ...
  private void exitGame() {
    ...
    intercepter.setGenerator(null);
    this.notifyObservers(
        new ObservationEvent(StatsMenu.TO_REPLAY.equals(source) ? TO_STATS : GAME_END));
    source = null;
  }
  ...
  public void setReplayGenerator(List<ArchiveInputEvent> arhive) {
    this.generator =
        new ReplayGenerator(this, arhive, judgementer.getAbsolutePosition(), judgementer.getSize());
    this.intercepter.setGenerator(generator);
  }
}
