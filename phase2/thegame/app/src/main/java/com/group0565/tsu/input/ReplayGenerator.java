...
public class ReplayGenerator extends InputGenerator {
  ...
  public ReplayGenerator(
      TsuEngine engine, List<ArchiveInputEvent> archive, Vector pos, Vector size) {
    super(engine);
  ...
  public List<InputEvent> update(long ms) {
    ...
    for (int i = passedPointer; i < archive.size(); i++) {
      ...
      if (i == passedPointer && event.endTime < getEngine().getCurrentTime()) passedPointer++;
      ...
      if (!used.contains(event) && event.startTime < getEngine().getCurrentTime()) {
        ...
        InputEvent inputEvent =
            new InputEvent(
                new Vector(
                    pos.getX() + (float) (size.getX() * event.position),
                    pos.getY() + size.getY() / 2f));
        inputEvents.add(inputEvent);
        active.put(event, inputEvent);
      } else if (event.endTime
          < getEngine()
              .getCurrentTime()) { // If the end time has passed, deactivate the input event.
        InputEvent inputEvent = active.remove(event);
        if (inputEvent != null) inputEvent.deactivate();
      }
    }
    return inputEvents;
  }
