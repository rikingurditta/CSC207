...
public class StatsMenu extends GameMenu implements Observable {
  ...
  public void init() {
    ...
    bgPaintCan =
        new ThemedPaintCan(SET, BackgroundPaintName)
            .init(getGlobalPreferences(), getEngine().getGameAssetManager());
    ...
    this.build()
        .add(
            BackButtonName,
            new Button(BUTTON_SIZE, ButtonBitmap.BackButton.getBitmap())
                .build()
                .addOffset(MARGIN)
        ...
        .add(
            BeatmapName,
            new ScrollingTextRenderer(
                () -> selectedBeatmapName == null ? "Beatmap not found" : selectedBeatmapName,
                ScrollTime,
                HoldTime,
                beatmapPaint,
                bgPaintCan,
        ...
        .add(
            HistoryListName,
            (historyList =
                    new HistoryList(this, new Vector(HISTORY_WIDTH, 0), this::getFilteredHistory))
                .build()
                .close())
        ...
        .add(
            TimeButtonName,
            new Button(BUTTON_SIZE, ButtonBitmap.TimeButton.getBitmap(), Sort.TimeDown)
                .build()
        ...
        .add(
            TimeUpButtonName,
            new Button(BUTTON_SIZE, ButtonBitmap.TimeUpButton.getBitmap(), Sort.TimeDown)
                .build()
        ...
        .add(
            TimeDownButtonName,
            new Button(BUTTON_SIZE, ButtonBitmap.TimeDownButton.getBitmap(), Sort.TimeUp)
                .build()
        ...
        .add(
            ScoreButtonName,
            new Button(BUTTON_SIZE, ButtonBitmap.ScoreButton.getBitmap(), Sort.ScoreDown)
                .build()
        ...
        .add(
            ScoreUpButtonName,
            new Button(BUTTON_SIZE, ButtonBitmap.ScoreUpButton.getBitmap(), Sort.ScoreDown)
                .build()
        ...
        .add(
            ScoreDownButtonName,
            new Button(BUTTON_SIZE, ButtonBitmap.ScoreDownButton.getBitmap(), Sort.ScoreUp)
                .build()
        ...
        .addCenteredAlignment(ScoreButtonName)
        .add(
            FullHistoryDisplayerName,
            new FullHistoryDisplayer(historyList::getSelectedObject, new Vector())
                .build()
  ...
  public void draw(Canvas canvas) {
    super.draw(canvas);
    if (getGlobalPreferences().getTheme() == Themes.LIGHT) canvas.drawRGB(255, 255, 255);
    else if (getGlobalPreferences().getTheme() == Themes.DARK) canvas.drawRGB(0, 0, 0);
  }
  ...
  private void observeReplay(Observable observable, ObservationEvent<SessionHitObjects> event) {
    if (event.isEvent(TO_REPLAY)) this.notifyObservers(event);
  }
  ...
  public void setSort(Sort sort) {
    this.sort = sort;
    if (filteredHistory != null) Collections.sort(filteredHistory, sort.comparator);
  ...
  private void refilter() {
    ...
    if (this.filteredHistory == null) this.filteredHistory = new ArrayList<>(history.size());
    else this.filteredHistory.clear();
