...
public class PauseMenu extends GameMenu implements Observable {
    ...
    public void init() {
        super.init();
        this.rim = new ThemedPaintCan(SET, RimName).init(getGlobalPreferences(), getEngine().getGameAssetManager());
        this.center = new ThemedPaintCan(SET, CenterName).init(getGlobalPreferences(), getEngine().getGameAssetManager());
        ...
        this.build()
            ...
            .add(ResumeName, new Button(BUTTON_SIZE, ButtonBitmap.ResumeButton.getBitmap(), RESUME).build()
                .registerObserver(this::observeButtons)
                .close())
            .addAlignment(Right, THIS, HCenter, -BUTTON_SIZE.getX()/2f)
            .addAlignment(VCenter, THIS, VCenter)

            .add(HomeName, new Button(BUTTON_SIZE, ButtonBitmap.HomeButton.getBitmap(), HOME).build()
                .registerObserver(this::observeButtons)
                .close())
