...
public class AndroidCanvas implements com.group0565.engine.interfaces.Canvas {
  ...
  public void drawText(String text, float x, float y, Paint paint) {
    ...
    } else throw new IllegalArgumentException("Only AndroidPaint can be drawn with.");
  }
  ...
  public void drawBitmap(Bitmap bitmap, Rect src, RectF dst, Paint paint) {
    if (paint instanceof AndroidPaint || paint == null) {
      ...
      } else throw new IllegalArgumentException("Only AndroidBitmap can be drawn.");
    } else throw new IllegalArgumentException("Only AndroidPaint can be drawn with.");
  }
  ...
  public void drawBitmap(Bitmap bitmap, Vector pos, Vector size, Vector bPos, Vector bSize) {
    if (bitmap instanceof AndroidBitmap) {
      ...
      aCanvas.drawBitmap(
          androidBitmap.getBitmap(),
          new Rect((int) bPos.getX(), (int) bPos.getY(), (int) bsum.getX(), (int) bsum.getY()),
          new RectF(pos.getX(), pos.getY(), psum.getX(), psum.getY()),
          null);
    } else throw new IllegalArgumentException("Only AndroidBitmap can be drawn.");
  }
